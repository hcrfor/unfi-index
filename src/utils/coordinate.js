...
/**
 * 🌟 어떠한 보정도 0% 적용하지 않고 엑셀 EPSG4326 열의 수치 그대로 파싱합니다.
 */
export function getDirectExcelCoordinates(item) {
  // 1순위: 엑셀 파일의 'EPSG4326' 열 원본 문자열 ("37.5251084278, 126.7168507097") 파싱
  if (item && item.epsg4326 && typeof item.epsg4326 === 'string' && item.epsg4326.includes(',')) {
    ...
    if (parts.length >= 2) {
      ...
      if (!isNaN(lat) && !isNaN(lng) && lat > 0 && lng > 0) {
        return {
          lat: lat, // 🌟 보정 0%! 엑셀 수치 100% 그대로!
          lng: lng, // 🌟 보정 0%! 엑셀 수치 100% 그대로!
          isValid: true,
          source: 'EXCEL_PURE_RAW',
        };
      }
    }
  }
